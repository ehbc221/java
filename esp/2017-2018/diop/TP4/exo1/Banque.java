public class Banque {
	...
	public Banque(int nombreMaxComptes) {
 ...
 * Ceci étant fait avec les appels de synchronized(), isInterrupted(), run(), start() et interrupt()
 *
 */
class ProcessBankCredit extends Thread {

	private Banque.Compte c;
	private int montant;

	public ProcessBankCredit(Banque.Compte c, int montant) {
		this.c = c;
 ...
 *
 */
class ProcessBankDebit extends Thread {

	private Banque.Compte c;
	private int montant;

	public ProcessBankDebit(Banque.Compte c, int montant) {
		this.c = c;
		this.montant = montant;
	}
